...
describe("Extraction Utilities", () => {
    ...
    describe("Recursive Content Fetching", () => {
        ...
        it("should respect timeout signal during recursive fetch", async () => {
            const { recursiveFetch } = await import("../../utils/extraction.js");
            const mockCtx = { log: vi.fn() };
            const visitedUrls = new Set();
            const results = [];
            const globalTimeoutSignal = { timedOut: true }; // Already timed out
            await recursiveFetch("https://example.com", 2, 1, visitedUrls, results, globalTimeoutSignal, mockCtx);
        ...
        it("should handle basic recursive fetch flow", async () => {
            ...
            await recursiveFetch("https://example.com", 1, 1, visitedUrls, results, globalTimeoutSignal, mockCtx);
        ...
        it("should fetch simpler content for deeper levels", async () => {
            ...
            await recursiveFetch("https://example.com/page1", 2, 2, // currentDepth > 1, should use fetchSimpleContent
            visitedUrls, results, globalTimeoutSignal, mockCtx);
            // Should have attempted to process the URL
            expect(mockCtx.log).toHaveBeenCalledWith("info", "[Depth 2] Fetching: https://example.com/page1");
        });
    });
});
