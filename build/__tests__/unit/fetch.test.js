...
describe("Fetch Utilities", () => {
    ...
    describe("Successful Content Fetching", () => {
        it("should fetch and extract HTML content successfully", async () => {
            ...
            const mockDocument = {
                title: "Test Page",
                body: {
                    textContent: "This is test content with enough characters to pass validation requirements for meaningful content extraction.",
                },
            };
            ...
            expect(result.textContent).toBe("This is test content with enough characters to pass validation requirements for meaningful content extraction.");
            expect(result.error).toBeUndefined();
            expect(mockAxiosGet).toHaveBeenCalledWith("https://example.com", expect.objectContaining({
                timeout: 8000,
        ...
        it("should handle plain text content", async () => {
            ...
            const mockDocument = {
                title: null,
                body: {
                    textContent: "This is plain text content with enough characters to pass the minimum length requirement for meaningful content.",
                },
            };
            mockJSDOM.mockImplementation(() => ({
                window: {
            ...
            expect(result.textContent).toBe("This is plain text content with enough characters to pass the minimum length requirement for meaningful content.");
            expect(result.error).toBeUndefined();
        });
        it("should use Readability for better content extraction when available", async () => {
            ...
            mockReadabilityParse.mockReturnValue({
                ...
                textContent: "This is a long article content that is meaningful and has enough characters to pass validation requirements.",
            });
            ...
            expect(result.textContent).toBe("This is a long article content that is meaningful and has enough characters to pass validation requirements.");
            expect(result.error).toBeUndefined();
        });
    });
    describe("Content Type Validation", () => {
        ...
        it("should accept HTML content types", async () => {
            ...
            const mockDocument = {
                title: null,
                body: {
                    textContent: "This is HTML content with sufficient length to pass validation requirements.",
                },
            };
            mockJSDOM.mockImplementation(() => ({
                window: {
            ...
            expect(result.error).toBeUndefined();
            expect(result.textContent).toBe("This is HTML content with sufficient length to pass validation requirements.");
        });
        it("should accept plain text content types", async () => {
            ...
            const mockDocument = {
                title: null,
                body: {
                    textContent: "Plain text content with sufficient length to pass validation requirements for meaningful content.",
                },
            };
            mockJSDOM.mockImplementation(() => ({
                window: {
            ...
            const result = await fetchSimpleContent("https://example.com/text", createMockContext());
            expect(result.error).toBeUndefined();
            expect(result.textContent).toBe("Plain text content with sufficient length to pass validation requirements for meaningful content.");
        });
    });
    ...
    describe("Error Handling", () => {
        ...
        it("should handle DNS resolution failures", async () => {
            ...
            const result = await fetchSimpleContent("https://invalid-domain-12345.com", createMockContext());
            expect(result.title).toBeNull();
            expect(result.textContent).toBeNull();
        ...
        it("should handle Readability parsing failures gracefully", async () => {
            ...
            const result = await fetchSimpleContent("https://example.com/readability-fail", createMockContext());
            // Should fall back to body text extraction
            expect(result.error).toBeUndefined();
            expect(result.title).toBe("Fallback Test");
            expect(result.textContent).toBe("This content has sufficient length to pass validation requirements.");
        });
    });
    describe("Edge Cases", () => {
