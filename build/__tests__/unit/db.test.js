...
describe("Database Utilities", () => {
    ...
    describe("Database Initialization", () => {
        it("should create required tables when initializing database", () => {
            ...
            dbModule.initializeDatabase(mockDb);
            expect(mockDb.exec).toHaveBeenCalledTimes(2);
            expect(mockDb.exec).toHaveBeenCalledWith(expect.stringContaining("CREATE TABLE IF NOT EXISTS chats"));
        ...
        it("should create tables with correct schema", () => {
            ...
            expect(mockDb.exec).toHaveBeenCalledWith(expect.stringContaining("created_at DATETIME DEFAULT CURRENT_TIMESTAMP"));
            ...
            expect(mockDb.exec).toHaveBeenCalledWith(expect.stringContaining("FOREIGN KEY (chat_id) REFERENCES chats(id)"));
        });
    });
    describe("Chat History Operations", () => {
        it("should retrieve chat history for a given chat ID", () => {
            ...
            testMessages.forEach((msg) => {
                dbModule.saveChatMessage(mockDb, chatId, msg);
            });
            // Retrieve chat history
            const history = dbModule.getChatHistory(mockDb, chatId);
            ...
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining("SELECT role, content FROM messages"));
        });
        it("should return empty array for non-existent chat ID", () => {
            const mockDb = createMockDatabase();
            const history = dbModule.getChatHistory(mockDb, "non-existent-chat");
    ...
    describe("Chat Message Saving", () => {
        it("should save user message to database", () => {
            ...
            const message = { role: "user", content: "Test user message" };
            dbModule.saveChatMessage(mockDb, chatId, message);
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining("INSERT OR IGNORE INTO chats"));
        ...
        it("should save assistant message to database", () => {
            ...
            const message = { role: "assistant", content: "Test assistant response" };
            dbModule.saveChatMessage(mockDb, chatId, message);
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining("INSERT OR IGNORE INTO chats"));
            expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO messages"));
        });
        it("should create chat record if it doesn't exist when saving message", () => {
            ...
            const message = { role: "user", content: "First message in new chat" };
            // Before saving, chat should not exist
            let history = dbModule.getChatHistory(mockDb, chatId);
            expect(history).toEqual([]);
            // Save message (should create chat)
            dbModule.saveChatMessage(mockDb, chatId, message);
            // After saving, chat should exist with the message
            history = dbModule.getChatHistory(mockDb, chatId);
        ...
        it("should prevent duplicate chat creation", () => {
            ...
            const message2 = { role: "assistant", content: "Response" };
            // Save two messages to the same chat
            dbModule.saveChatMessage(mockDb, chatId, message1);
            dbModule.saveChatMessage(mockDb, chatId, message2);
            // Should have called INSERT OR IGNORE twice but should only create chat once
            const prepareCalls = mockDb.prepare.mock.calls;
            const insertOrIgnoreCalls = prepareCalls.filter((call) => call[0]?.includes("INSERT OR IGNORE INTO chats"));
    ...
    describe("Edge Cases", () => {
        it("should handle special characters in message content", () => {
            ...
            dbModule.saveChatMessage(mockDb, chatId, messageWithSpecialChars);
            const history = dbModule.getChatHistory(mockDb, chatId);
            expect(history).toHaveLength(1);
        ...
        it("should handle empty message content", () => {
            ...
            dbModule.saveChatMessage(mockDb, chatId, emptyMessage);
            const history = dbModule.getChatHistory(mockDb, chatId);
            expect(history).toHaveLength(1);
