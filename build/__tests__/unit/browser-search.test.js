...
vi.mock("../../utils/puppeteer.js", () => ({
    ...
    retryOperation: vi.fn().mockImplementation(async (_ctx, fn) => fn()),
}));
// Mock logging
...
describe("Browser and Search Modules", () => {
    ...
    describe("BrowserManager", () => {
        ...
        it("should wait for search input successfully", async () => {
            const browserManager = new BrowserManager();
            vi.mocked(puppeteerUtils.waitForSearchInput).mockResolvedValue('textarea[placeholder*="Ask"]');
        ...
        it("should determine if browser is ready", () => {
            ...
            browserManager.browser = mockBrowser;
            browserManager.page = mockPage;
        ...
        it("should handle level 1 recovery (page refresh)", async () => {
            ...
            expect(puppeteerUtils.recoveryProcedure).toHaveBeenCalledWith(expect.objectContaining({}), error);
        });
    ...
    describe("SearchEngine", () => {
        it("should execute search successfully", async () => {
            ...
            vi.mocked(puppeteerUtils.retryOperation).mockImplementation(async (_ctx, _fn) => {
        ...
        it("should execute search with retry mechanism", async () => {
            ...
            vi.mocked(puppeteerUtils.retryOperation).mockImplementation(async (_ctx, fn) => {
                return await fn();
            });
            const result = await searchEngine.performSearch(query);
