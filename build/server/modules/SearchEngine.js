...
export class SearchEngine {
    ...
    async performSearch(query, model, attachments) {
        ...
        const operationTimeout = setTimeout(() => {
            ...
            this.browserManager.performRecovery().catch((err) => {
    ...
    async executeSearch(page, selector, query) {
    ...
    async waitForCompleteAnswer(page) {
    ...
    async extractCompleteAnswer(page) {
        ...
        const answerPromise = page.evaluate(async () => {
            ...
            const getAnswer = () => {
                ...
                const answerText = elements.map((el) => el.innerText.trim()).join("\n\n");
    ...
    async extractFallbackAnswer(page) {
        try {
            return await page.evaluate(() => {
                ...
                for (const selector of contentSelectors) {
                    ...
                    if (textElements.length > 0) {
                        ...
                        return topElements.map(el => el.innerText.trim()).join('\n\n');
                    }
                }
    ...
    async performDeepResearch(query, attachments) {
        const operationTimeout = setTimeout(() => {
            logError("Global operation timeout reached, initiating recovery...");
            this.browserManager.performRecovery().catch((err) => {
                logError("Recovery after timeout failed:", {
        ...
        try {
            ...
            return await retryOperation(ctx, async () => {
                ...
                const activated = await page.evaluate(() => {
                    ...
                    const target = items.find((el) => el.innerText.includes("Deep research"));
                    if (target) {
                        target.click();
                        return true;
                    }
    ...
    async uploadAttachments(page, filePaths) {
        try {
    ...
    async selectModel(page, modelName) {
        try {
            ...
            const selected = await page.evaluate((targetModel) => {
                const items = Array.from(document.querySelectorAll('[role="menuitem"], [role="option"], button, span'));
                const target = items.find((el) => el.innerText.toLowerCase().includes(targetModel.toLowerCase()));
                if (target) {
                    target.click();
                    return true;
                }
