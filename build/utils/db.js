 ...
 */
export function getChatHistory(db, chatId) {
    const messages = db
 ...
 */
export function saveChatMessage(db, chatId, message) {
    db.prepare("INSERT OR IGNORE INTO chats (id) VALUES (?)").run(chatId);
    db.prepare("INSERT INTO messages (chat_id, role, content) VALUES (?, ?, ?)").run(chatId, message.role, message.content);
}
