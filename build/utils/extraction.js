...
function detectAndRewriteGitHubUrl(originalUrl, ctx) {
    try {
...
async function performContentTypeCheck(extractionUrl, isGitHubRepo, originalUrl, ctx) {
    if (isGitHubRepo) {
    ...
    try {
        ...
        ctx.log("warn", `HEAD request failed for ${extractionUrl}: ${headError instanceof Error ? headError.message : String(headError)}. Proceeding with Puppeteer.`);
    }
    return null;
}
async function initializePageIfNeeded(ctx) {
    let page = ctx.page;
...
async function navigateToUrl(page, extractionUrl, originalUrl, ctx) {
...
async function waitForGitHubContent(page, isGitHubRepo, ctx) {
    if (!isGitHubRepo)
        return;
...
async function extractGitHubContent(page, isGitHubRepo, originalUrl, pageTitle, ctx) {
    if (!isGitHubRepo)
        return null;
...
function extractGeneralContent(dom, originalUrl, pageTitle, ctx) {
...
async function extractFallbackContent(page, originalUrl, pageTitle, ctx) {
    ...
    const fallbackResult = await page.evaluate(() => {
        ...
        const elementsToRemove = bodyClone.querySelectorAll('nav, header, footer, aside, script, style, noscript, button, form, [role="navigation"], [role="banner"], [role="contentinfo"], [aria-hidden="true"]');
    ...
    if (fallbackResult) {
        ctx.log("info", `Fallback extracted content (${fallbackResult.text.length} chars) using selector: ${fallbackResult.selector}`);
        return {
            url: originalUrl,
            title: pageTitle,
...
function formatExtractionError(error, extractionUrl, originalUrl) {
 ...
 */
export async function fetchSinglePageContent(url, ctx) {
    ...
    const contentTypeError = await performContentTypeCheck(extractionUrl, isGitHubRepo, originalUrl, ctx);
    if (contentTypeError) {
    ...
    try {
        ...
        const gitHubResult = await extractGitHubContent(page, isGitHubRepo, originalUrl, pageTitle, ctx);
        if (gitHubResult) {
            return gitHubResult;
        }
        ...
        ctx.log("error", `Error extracting content from ${extractionUrl}: ${error instanceof Error ? error.message : String(error)}`);
 ...
 */
export async function extractSameDomainLinks(page, baseUrl) {
    try {
 ...
 */
export async function recursiveFetch(startUrl, maxDepth, currentDepth, visitedUrls, results, globalTimeoutSignal, ctx) {
    ...
    try {
        ...
        ctx.log("error", `[Depth ${currentDepth}] Error fetching ${startUrl}: ${error instanceof Error ? error.message : String(error)}`);
    ...
    if (currentDepth < maxDepth && !pageResult.error && linksToExplore.length > 0) {
        ...
        const promises = linksToFollow.map((link) => {
            if (globalTimeoutSignal.timedOut)
                return Promise.resolve();
            return recursiveFetch(link.url, maxDepth, currentDepth + 1, visitedUrls, results, globalTimeoutSignal, ctx);
        });
