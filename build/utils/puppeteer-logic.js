 ...
 */
export function determineRecoveryLevel(error, context) {
    if (!error)
        return 1;
...
export function analyzeError(error) {
    ...
    return {
        ...
        isConnection: lowerMsg.includes("net::") || lowerMsg.includes("connection") || lowerMsg.includes("network"),
        isDetachedFrame: lowerMsg.includes("frame") ||
 ...
 */
function generateRetryJitter(maxJitter) {
 ...
 * Note: Math.random() is safe here - only used for timing distribution, not security
 */
function generateConnectionDelay() {
 ...
 * Note: Math.random() is safe here - only used for timing distribution, not security
 */
function generateDetachedFrameDelay() {
    return 10000 + Math.random() * 5000; // 10-15 seconds
}
/**
 * Calculate retry delay with exponential backoff and jitter
 */
export function calculateRetryDelay(attemptNumber, errorAnalysis, maxDelay = 30000) {
    let baseDelay;
 ...
 */
export function generateBrowserArgs(userAgent) {
    return [
 ...
 */
export function getSearchInputSelectors() {
    return [
 ...
 */
export function getCaptchaSelectors() {
    return [
 ...
 */
export function validateNavigationUrl(url, expectedDomain) {
    try {
 ...
 */
export function isNavigationFailure(url, expectedUrl) {
    if (!url || url === "N/A")
        return true;
    if (expectedUrl) {
        try {
