...
import { analyzeError, calculateRetryDelay, determineRecoveryLevel, generateBrowserArgs, getCaptchaSelectors, getSearchInputSelectors, } from "./puppeteer-logic.js";
export async function initializeBrowser(ctx) {
    ...
    try {
        ...
        if (!headless) {
            browserArgs = browserArgs.filter(arg => !arg.includes('--disable-gpu') &&
        ...
        ctx.setPage(null);
        throw new Error(`Page not initialized: ${error instanceof Error ? error.message : String(error)}`);
    }
    finally {
...
async function performInitialNavigation(page) {
    try {
        ...
        logWarn(`Navigation issue detected: ${gotoError instanceof Error ? gotoError.message : String(gotoError)}`);
    }
}
...
async function waitForAndValidateSearchInput(ctx) {
    const { page } = ctx;
    if (!page)
...
async function validateFinalPageState(page) {
...
async function handleNavigationFailure(page, error) {
    ...
    if (shouldCaptureScreenshot) {
        try {
            if (page && !page.isClosed()) {
                ...
                const screenshotPath = `debug_navigation_failed_${timestamp}.png`;
...
export async function navigateToPerplexity(ctx) {
    const { page } = ctx;
    if (!page)
        throw new Error("Page not initialized");
    try {
...
export async function setupBrowserEvasion(ctx) {
    const { page } = ctx;
    if (!page)
        return;
...
export async function waitForSearchInput(ctx, timeout = CONFIG.SELECTOR_TIMEOUT) {
...
export async function checkForCaptcha(ctx) {
    const { page } = ctx;
    if (!page)
        return false;
...
async function performPageRefresh(ctx) {
...
async function performNewPageCreation(ctx) {
...
async function cleanupPage(ctx) {
    if (!ctx.page)
        return;
    try {
        if (!ctx.page.isClosed())
...
async function performFullBrowserRestart(ctx) {
...
export async function recoveryProcedure(ctx, error) {
    ...
    if (opId % 5 === 0) {
        cleanupOldDebugScreenshots().catch((err) => logWarn(`Failed to cleanup old screenshots: ${err}`));
    }
    try {
 ...
 */
function generateCriticalErrorDelay() {
    return 10000 + Math.random() * 5000; // 10-15 seconds
}
async function handleDetachedFrameError(ctx, error) {
    const errorMsg = error.message;
    logError(`Detached frame or protocol error detected ('${errorMsg.substring(0, 100)}...'). Initiating immediate recovery.`);
    await recoveryProcedure(ctx, error);
    const criticalWaitTime = generateCriticalErrorDelay();
    logInfo(`Waiting ${Math.round(criticalWaitTime / 1000)} seconds after critical error recovery...`);
    await new Promise((resolve) => setTimeout(resolve, criticalWaitTime));
}
async function handleCaptchaDetection(ctx) {
...
async function handleTimeoutError(ctx, consecutiveTimeouts) {
...
async function handleNavigationError(ctx, consecutiveNavigationErrors) {
 ...
 * Note: Math.random() is safe here - only used for timing distribution, not security
 */
function generateConnectionErrorDelay() {
    return 15000 + Math.random() * 10000; // 15-25 seconds
}
async function handleConnectionError(ctx) {
 ...
 * Note: Math.random() is safe here - only used for timing distribution, not security
 */
function generateNavigationFailureDelay() {
    return 10000 + Math.random() * 5000; // 10-15 seconds
}
async function handleRetryNavigation(ctx, attemptNumber) {
    try {
        ...
        logInfo(`Navigation failed, waiting ${Math.round(navFailWaitTime / 1000)} seconds before next attempt...`);
...
async function handleRetryError(ctx, error, attemptNumber, consecutiveTimeouts, consecutiveNavigationErrors) {
...
export async function retryOperation(ctx, operation, maxRetries = CONFIG.MAX_RETRIES) {
    let lastError = null;
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            hadRecovery = true;
            const retryResult = await handleRetryError(ctx, lastError, i, consecutiveTimeouts, consecutiveNavigationErrors);
 ...
 */
async function cleanupOldDebugScreenshots() {
    try {
 ...
 */
function trackRecoverySuccess(ctx) {
    ...
    cleanupOldDebugScreenshots().catch((err) => logWarn(`Failed to cleanup screenshots after successful recovery: ${err}`));
}
export function resetIdleTimeout(ctx) {
    if (ctx.idleTimeout) {
